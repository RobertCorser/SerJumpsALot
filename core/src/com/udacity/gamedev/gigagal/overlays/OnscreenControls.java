...
public class OnscreenControls extends InputAdapter {
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (viewportPosition.x < viewport.getWorldWidth() / 2) {
            moveLeftPointer = pointer;

            if (gigaGal.getWalkState() == Enums.WalkState.NOT_WALKING) {
                gigaGal.setWalkState(Enums.WalkState.WALKING);
                gigaGal.setFacing(Enums.Direction.LEFT);
            } else if (gigaGal.getFacing() == Enums.Direction.RIGHT) {
                gigaGal.setWalkState(Enums.WalkState.NOT_WALKING);
            } else {
                if (gigaGal.getFacing() == Enums.Direction.LEFT) {
            ...
            if (gigaGal.getWalkState() == Enums.WalkState.NOT_WALKING) {
                gigaGal.setWalkState(Enums.WalkState.WALKING);
                gigaGal.setFacing(Enums.Direction.RIGHT);
            } else if (gigaGal.getFacing() == Enums.Direction.LEFT) {
                gigaGal.setWalkState(Enums.WalkState.NOT_WALKING);
            } else {
                if (gigaGal.getFacing() == Enums.Direction.RIGHT) {
                    gigaGal.jumpButtonPressed = true;
                }
            }
